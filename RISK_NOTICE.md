...
- Saving a password lets this app retrieve it from macOS Keychain for later input. The CLI receives only the result, but the destination app receives the password. If you allow saved-password input and disable Ask me before each password use, software on your account can request that input without an additional app confirmation. A field marked secure can still belong to an untrusted app. Inspect the target before approving.
- Allowed actions and per-use approval are app controls, not a security boundary against software that already controls your account, preferences, or desktop. AI agents must leave approval to you. Installing a skill does not grant any of these permissions.

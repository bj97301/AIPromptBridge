...
- Skill discovery, installation, and export are available before acknowledgment. They share the GUI installer, preserve customized copies, and do not change control permissions or password settings. A custom folder is an explicit installation destination for this app's skill only.
...
- Button presses, manual field input, saved-password input, and capture/OCR have independent native permission checks. Input and capture are opt-in. Ask first defaults to on for both input sources; native approval is bound to a single target and expires. IPC exposes polling and cancellation, never approval.
- Saved credentials use this app's own local macOS Keychain item with a creating-app access list. They are retrieved only after authorization and target validation and are never returned over IPC. The receiving app gets the value. Keychain protects storage, but a malicious destination can still misuse input. [Details](docs/PASSWORDS.md).
